...
\textbf{Определение:} Точка \(b \in \overline{\mathbb{C}}\) называется
...
\textbf{Теорема:} Любая последовательность
\(\{ z_{n} \}, z_{n}\in \mathbb{C}\) имеет предельную точку в
\(\overline{\mathbb{C}}\).
...
\textbf{Определение:}
\(\lim_{ n \to \infty }z_{n}=\infty\Leftrightarrow\forall\varepsilon>0 \ \exists n_{\varepsilon}: \forall n\geq n_{\varepsilon}\to |z_{n}|>\varepsilon\).
