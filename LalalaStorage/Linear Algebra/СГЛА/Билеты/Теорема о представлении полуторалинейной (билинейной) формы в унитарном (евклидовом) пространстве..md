---
title:  Теорема о представлении полуторалинейной (билинейной) формы в унитарном (евклидовом) пространстве.
created: 2026-01-24
tags:
links:
---
...
$\blacktriangleleft$
Пусть $f(x,y)$ - П.Ф. в $U$. Фиксируем $y$ $\implies f(x,y)$ - линейная форма в $U$ $\implies$
[[Определение линейной формы. Примеры. Вид линейной формы в базисе#^eb5dbd|По теореме 8]] $\exists!h_{y}: \ \forall x\in U \ \ f(x,y)=(x,h_{y})$ 
Т.е. $\forall y\in U \ \ \exists!h_{y}$ или мы задали оператор $A$ в $U: Ay=h_{y}$
Итак, $\exists A:U\to U:$ $\forall x,y\in U \ \ f(x,y)=(x,Ay)(*)$ 
$\forall x,y,z\in U \ \forall\lambda \in \mathbb{C}:$ ^1f8af9
1. $f(x,y+z)=(x,A(y+z))$
