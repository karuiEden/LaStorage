...
| **Датасет** - набор примеров, на которой обучают модель. Датасет состоит из двух частей: **объектов** и **ответов**.

#### Типы задач
1. Supervised Learning (Обучение с учителем)
	1. $\mathbb{Y}=\mathbb{R}$ или $\mathbb{Y}=\mathbb{R}^{M}$ - регрессия. Примеры: предсказание продолжительности на каршеринге, спрос на товар в конкретный день или погода в вашем городе на завтра(формирует вектор).
	2. $\mathbb{Y}=0,1$ - бинарная классификация. Пример: сдаст ли студент сессию, кликнет ли пользователь по ссылке и т.д.
	3. $\mathbb{Y}=1,\dots,K$ - многоклассовая классификация. Пример: определение предметной области статьи.
	4. $\mathbb{Y}=0,1^{K}$ - многоклассовая классификация с пересекающимися классами. Пример: определение тегов для ресторана.
	5. $\mathbb{Y}$ - конечное упорядоченное множество - ранжирование. Пример: задача ранжирования поисковой выдачи.
...
![[Pasted image 20250114155656.png]]
| Данные, представленные в таком виде, называются **табличными**.
