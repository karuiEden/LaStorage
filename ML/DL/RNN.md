...
Ничто не мешает использовать всю последовательность входных данных одной RNN в качестве входной последовательности для другой. Многослойный RNN состоят из нескольких сетей, где выход одного слоя служит входом для последующего слоя, как показано ниже. Многослойные RNN, как правило, превосходят однослойные сети. Одна из причин этого успеха, по-видимому, заключается в том, что сеть формирует представления на разных уровней абстракции между слоями. Подобно тому, как на ранних этапах человеческой зрительной системы происходит обнаружение границ, которые затем используются для поиска больших областей и форм, начальные слои многослойных сетей могут формировать представления, которые служат полезными абстракциями для последующих слоев - представления, которые может оказаться трудно сформировать в одной RNN. Оптимальное количество многослойных RNN зависит от конкретного приложения и каждого обучающего набора данных. Однако с увеличением количества слоев быстро растут и затраты на обучение.

![[Pasted image 20251223181114.png]]
## Двунаправленные RNN

RNN использует информацию с левого(предыдущего) контекста, чтобы делать предсказания во время $t$. Но во многих приложениях мы имеем доступ ко всей входной последовательности. Один способ это сделать, использовать 2 отдельные RNN, одну модель слева направо, вторую справа на лево, и конкетировать их представления.

В лево-направо RNN скрытое состояние в данное время $t$ представляет все, что сеть знает о последовательности до этого момента времени. Это состояние это функция ввода $x_{1},x_{2},\dots,x_{t}$ и представляет контекст сети слева от текущего момента времени:
$$
\mathrm{h}_{t}^{f} = \mathrm{RNN_{forward}}(x_{1},\dots,x_{t})
$$
Новая нотация $\mathrm{h}_{t}^{f}$ просто совпадает с обычным скрытым состоянием RNN момента времени $t$.
Чтобы получить преимущество контекста справа от текущего ввода, мы можем обучить модель RNN на обратной последовательности входных данных. С этим подходом, скрытое состояние в момент времени $t$ представляет информацию о последовательности справа от текущего ввода:
$$
\mathrm{h}_{t}^{b}=\mathrm{RNN_{backforward}}(x_{t},\dots,x_{n})
$$
*Двунаправленная RNN* комбинирует две независимых RNN, где одна обрабатывает информацию с начала до конца, а вторая - с конца до начала. Затем конкетируем два представления, вычисленных сетями в единый вектор, который отражает как левый, так и правый контекст входных данных в каждый момент времени:
$$
\mathrm{h}_{t} = \mathrm{h}_{t}^{f} \oplus \mathrm{h}_{t}^{b}
$$

![[Pasted image 20251223183023.png]]

Другие способы объединения прямого и обратного контекстов включают поэлементное сложение или умножение. Таким образом, выходные данные на каждом шаге времени содержат информацию слева и справа от текущих входных данных.